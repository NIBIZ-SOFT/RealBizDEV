...
$TheEntityName = SQL_Select($Entity = "{$Entity}", $Where = "{$Entity}{$UniqueField} = '{$_POST["{$UniqueField}"]}' AND {$Entity}ID <> {$_REQUEST[$Entity."ID"]}");
...
if ($ErrorUserInput["_Error"]) {
    ...
    $TheEntityName = SQL_InsertUpdate(
        ...
        $TheEntityNameData = array(
            ...
            "Unit" => $_POST["Unit"],
            "IncomeExpenseTypeName" => GetIncomeExpenseTypeName($_POST["IncomeExpenseTypeID"]),
            "IncomeExpenseTypeID" => $_POST["IncomeExpenseTypeID"],
            "InventorySubCategory" => $_POST["InventorySubCategory"],
            "GLCode" => $_POST["GLCode"],


            "{$Entity}IsType" => $_POST["{$Entity}IsType"],
            "{$Entity}IsStock" => $_POST["{$Entity}IsStock"],
