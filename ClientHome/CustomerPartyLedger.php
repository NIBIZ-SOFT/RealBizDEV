...
$GetCustomerID = SQL_Select("Customer","Phone='{$_SESSION["CustomerPhone"]}'","",true);
if (empty($GetCustomerID)) {
        echo '
        <script>
            alert( "No Transaction available Available " );
            history.go(-1);
        </script>
    ';
    exit;
}
$_REQUEST["CustomerID"] = $GetCustomerID["CustomerID"];
