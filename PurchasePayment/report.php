    ...
    <div class="container">
        ...
        <div class="print-rule-area">
            ...
            <div class="print-rule">
                <h6> 2. Mode of payment: <span style="font-weight: normal">Payment will be made through bank deposit / Account payee cheque</span></h6>
                <h6> 3. Date of Delivery: <span style="font-weight: bold">' . HumanReadAbleDateFormat($DateOfDelevery) . '</span></h6>
                <h6> 4. Validity: <span style="font-weight: normal">This order will be valid for 05 (Five) days from the date of issuing this order</span></h6>
...
// Calculate Total, Paid, Due Amount
$purchaseInfo = mysql_query("SELECT PurchaseAmount FROM tblpurchase WHERE PurchaseID = '".intval($PurchaseId)."'");
$purchaseRow = mysql_fetch_assoc($purchaseInfo);
$totalAmount = isset($purchaseRow['PurchaseAmount']) ? floatval($purchaseRow['PurchaseAmount']) : 0;
mysql_free_result($purchaseInfo);

$paidResult = mysql_query("SELECT SUM(dr) as totalDr FROM tbltransaction WHERE PurchaseID = '".intval($PurchaseId)."'");
$paidRow = mysql_fetch_assoc($paidResult);
$totalDr = isset($paidRow['totalDr']) ? floatval($paidRow['totalDr']) : 0;
mysql_free_result($paidResult);

$dueAmount = $totalAmount - $totalDr;

// Payment History Table (no card/box)
$MainContent .= '
<div class="container">
...
$historyResult = mysql_query("SELECT TransactionID, Date, BankCashName, HeadOfAccountName, ChequeNumber, dr, Description, Image FROM tbltransaction WHERE PurchaseID = '{$_REQUEST["ID"]}' ORDER BY TransactionID DESC");
while ($row = mysql_fetch_assoc($historyResult)) {
    $imgHtml = '';
    if (!empty($row['Image'])) {
        $imgHtml = '<a href="'.$row['Image'].'" target="_blank">View</a>';
    }
    $MainContent .= '
        <tr>
...
// Payment Summary (no box, just values) - moved below Payment History table
$MainContent .= '
<div class="container mt-4 mb-3">
    <div class="row">
        ...
        <div class="col text-center">
            <strong>Paid Amount:</strong> '.number_format($totalDr,2).'
        </div>
        <div class="col text-center">
            <strong>Due Amount:</strong> '.number_format($dueAmount,2).'
        </div>
    </div>
</div>
';
