...
if (!empty($_REQUEST["ProductID"])) {
    ...
    $categories = SQL_Select("category where CategoryID=116");
    ...
    foreach ($categories as $category) {
        $salesProjctDetails = SQL_Select("sales where ProductID='{$_REQUEST["ProductID"]}' and SalesDate BETWEEN '{$_REQUEST["FromDate"]}' and '{$_REQUEST["ToDate"]}' ORDER BY `SalesDate` Asc");
...
//        Due payment
        ...
        ';
            ...
            $sl++;
            $grossQuentity += $salesProjctDetail["Quantity"];
            $TotalDeductionCharge += $salesProjctDetail["Discount"];
...
<div style="width: 95%; margin: auto">
    ...
    <div class="projectName text-center m-b-30 m-t-30">
        <h4 style="font-weight: bold">Package Wise Party Ledger Summary</h4>
    ...
    <table style="font-size: 16px" class="table table-bordered table-hover table-fixed table-sm">
        ...
        <tbody>
            ...
            <tr style="font-weight: bold;">
                ...
                <td class="text-right">' . $grossQuentity. '</td>
