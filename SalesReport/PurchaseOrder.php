...
$ProjectDetails = SQL_Select("category");
...
foreach ($ProjectDetails as $ProjectDetail){
    ...
    if ( empty($_REQUEST["CategoryID"]) and empty($_REQUEST["FromDate"]) and empty($_REQUEST["ToDate"])  ){
        $purchaserequisitionDetails=SQL_Select("purchase where CategoryID={$ProjectDetail["CategoryID"]} and Confirm='Confirm' ");
...
//        echo "<pre>";
//        print_r($purchaserequisitionDetails);
//        die();

        foreach ($purchaserequisitionDetails as $purchaserequisitionDetail){

            $CategoryName=$purchaserequisitionDetail["CategoryName"];
            $RequisitionConfirmID=$purchaserequisitionDetail["confirmRequisitonName"];
            $PurchaseConfirmID=$purchaserequisitionDetail["PurchaseConfirmID"];
            $EmployeeName=$purchaserequisitionDetail["MediaName"];
            ...
            foreach ($Items as $Item){
                ...
                $trHtml .='
                    <tr>
                        ...
                        <td>'.$PurchaseConfirmID.'</td>
                        <td>'.HumanReadAbleDateFormat($RequiredDate).'</td>
                        <td>'.$expenseHead.'</td>
                        <td class="text-right">'.BangladeshiCurencyFormat($requisitionQty).'</td>
                        <td  class="text-right">'.BangladeshiCurencyFormat($totalrequisitionRate).'</td>
                        <td  class="text-right">'.BangladeshiCurencyFormat($totalrequisitionAmount).'</td>
        ...
        $purchaserequisitionDetails=SQL_Select("purchase where CategoryID={$ProjectDetail["CategoryID"]} and Confirm='Confirm' and IssuingDate BETWEEN '{$_REQUEST["FromDate"]}' AND  '{$_REQUEST["ToDate"]}' ");
    ...
    if (empty($CategoryName) or empty($totalrequisitionQty)){
        continue;
    }
